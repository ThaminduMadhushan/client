...
export default function BinTypeList() {
  ...
  const AdminId = async (userId) => {
    try {
      ...
      fetchBinTypes(); // Fetch orders once admin id is set
    } catch (error) {
  ...
  const fetchBinTypes = async () => {
    try {
      const response = await fetch(
        `http://localhost:3001/api/binType`
      );
      if (!response.ok) {
  ...
  const deleteUser = async (id) => {
    try {
      ...
      if (confirmed.isConfirmed) {
        const response = await fetch(
          `http://localhost:3001/api/binType/${id}`,
          {
            method: "DELETE",
          }
        );
                      ...
                      <TableCell align={"left"}>
                        <Stack spacing={2}>
                          <EditIcon
                            ...
                            onClick={() => handleOpenEditModal(row.type_id)} // Pass the bin ID to the edit modal
                          />
                          <DeleteIcon
                            style={{
                              fontSize: "20px",
                            ...
                            className="cursor-pointer"
                            onClick={() => deleteUser(row.type_id)}
                          />
                        </Stack>
                      </TableCell>
                    </TableRow>
