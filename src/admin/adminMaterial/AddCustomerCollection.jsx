...
//   const [selectedSupplier, setSelectedSupplier] = useState(null); // Store selected supplier object
...
//       if (!response.ok) {
//         throw new Error("Failed to add customer collection");
//       }
...
//       Swal.fire("Error!", "Failed to add the customer collection.", "error");
//       closeEvent();
//     }
//   };
...
//   const fetchCollector = async (supplierId) => {
//     try {
//       const response = await fetch(`http://localhost:3001/api/supplier/user/${supplierId}`);
//       if (!response.ok) {
...
//       await fetchCollector(selectedSupplier.supplier_id);
...
//             onChange={(event, newValue) => {
//               setSelectedSupplier(newValue);
//               setCollectorId(null); // Reset collector ID when supplier changes
//             }}
//             id="supplier-autocomplete"
//             options={suppliers}
//             getOptionLabel={(option) => option.firstname}
...
function AddCustomerCollection({ closeEvent }) {
  ...
  //     // Call fetchCollector asynchronously and wait for its response
  //     const collectorId = await fetchCollector(selectedSupplier.supplier_id);
  
  //     // Ensure collectorId is retrieved successfully
  //     if (!collectorId) {
  //       throw new Error("Collector ID not found");
  //     }
  ...
  const fetchCollector = async (supplierId) => {
    try {
      ...
      const data = await response.json();
      if (!data.user_id) {
        throw new Error("Collector ID not found in response");
      }
      setCollectorId(data.user_id);
    } catch (error) {
      console.error("Error fetching collector:", error);
