...
//         `http://localhost:3001/api/salary/collection/${role}?startDate=${startDate.toISOString().split('T')[0]}&endDate=${endDate.toISOString().split('T')[0]}`
//       );
//       if (!response.ok) {
...
//       acc[curr.user_id] = acc[curr.user_id] || { total: 0, role: curr.role };
//       acc[curr.user_id].total += curr.quantity;
//       return acc;
//     }, {});

//     Object.keys(groupedCollections).forEach((userId) => {
//       const totalCollection = groupedCollections[userId].total;
//       const userRole = groupedCollections[userId].role;

//       const param = parameters.find((param) => param.role.toLowerCase() === userRole.toLowerCase());
...
//           epf: param.epf,
//           bonus: param.bonus,
//           monthly_target: param.monthly_target,
//           unit_target_bonus: param.target_bonus,
...
//     try {
//       const response = await fetch(
//         `http://localhost:3001/api/salary/collection/${role}?startDate=${startDate}&endDate=${endDate}`
//       );
//       if (!response.ok) {
//         throw new Error(`Failed to fetch ${role} collection`);
//       }
