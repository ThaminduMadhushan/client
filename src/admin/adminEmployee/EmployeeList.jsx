...
export default function EmployeeList() {
  ...
  const changeStatus = async (user_id) => {
    try {
      const confirmed = await Swal.fire({
        title: "Are you sure?",
        text: "You want to set this employee as inactive!",
        icon: "warning",
      ...
      if (confirmed.isConfirmed) {
        const response = await fetch(
          `http://localhost:3001/api/employees/status/${user_id}`,
          {
            method: "PUT",
          }
        );
        ...
        const newRows = rows.map((row) =>
          row.user_id === user_id ? { ...row, status: "inactive" } : row
        );
        setRows(newRows);

        Swal.fire("Updated!", "Employee status has been set to inactive.", "success").then(
          () => {
  ...
  return (
    <>
      ...
      >
        <Box sx={style}>
          <AddEmployees closeEvent={handleCloseAddModal} />
        </Box>
      </Modal>
