...
export default function AcceptOrder() {
  ...
  const fetchOrders = async () => {
    try {
      const response = await fetch(
        `http://localhost:3001/api/admin/completed`
      );
      if (!response.ok) {
  ...
  const finishedOrder = async (id) => {
    try {
      ...
      if (confirmed.isConfirmed) {
        const response = await fetch(`http://localhost:3001/api/orders/finished/${id}`, {
          method: "PUT",
          headers: {
        ...
        if (!response.ok) {
          throw new Error(responseData.error || "Failed to finish order");
        }
  
        const newRows = rows.filter((row) => row.id !== id);
        setRows(newRows);
  
        Swal.fire("Finished!", "The order has been Finished.", "success");
        window.location.reload();
      }
    } catch (error) {
      console.error("Error finishing order:", error);
      Swal.fire("Error!", error.message || "Failed to Finish the order.", "error");
    }
  };
                    ...
                    <TableCell align={"left"}>
                      <Stack spacing={2}>
                        <CheckCircleIcon
                          ...
                          onClick={() => finishedOrder(row.order_id)}
                        />
                      </Stack>
                    </TableCell>
                  </TableRow>
