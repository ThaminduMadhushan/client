...
export default function Person() {
  ...
  const [bailerId, setBailerId] = useState('');
  const [bailerFirstname, setBailerFirstname] = useState('');
  const [bailerLastname, setBailerLastname] = useState('');
  ...
  const fetchBailerDetails = async (userId) => {
    try {
      const response = await fetch(`http://localhost:3001/api/bailer/${userId}`);
      if (!response.ok) {
        throw new Error('Failed to fetch bailer details');
      }
      const data = await response.json();
      setBailerId(data.bailer_id);
      setBailerFirstname(data.firstname);
      setBailerLastname(data.lastname);
      setAboutText(data.about);
    } catch (error) {
      setError('Failed to fetch bailer details');
    }
  };
  ...
  const handleSaveAbout = async () => {
    try {
      setLoading(true);
      await axios.put(`http://localhost:3001/api/bailer/edit/about/${user.id}`, { about: aboutText });
      setIsEditing(false);
    } catch (error) {
          ...
          <Grid container spacing={2} alignItems="center">
            <Grid item xs={6}>
              <Stack spacing={1}>
                ...
                <Typography variant="body1">{bailerFirstname}</Typography>
              </Stack>
            </Grid>
            <Grid item xs={6}>
              <Stack spacing={1}>
                ...
                <Typography variant="body1">{bailerLastname}</Typography>
              </Stack>
            </Grid>
            <Grid item xs={12}>
              <Stack spacing={1}>
