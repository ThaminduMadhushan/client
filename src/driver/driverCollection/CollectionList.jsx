...
export default function CollectionList() {
  ...
  const handleOpenEditModal = (collectionId) => {
  ...
  const driverId = async (userId) => {
    try {
      ...
      fetchCollections(data.driver_id); // Assuming data is an array of objects
    } catch (error) {
  ...
  const fetchCollections = async (userId) => {
    try {
      const response = await fetch(
        `http://localhost:3001/api/driver/collection/${userId}`
      );
      if (!response.ok) {
                    ...
                    <TableCell align={"center"}>
                      <Stack spacing={2}>
                        <EditIcon
                          ...
                          onClick={() => handleOpenEditModal(row.collection_id)} // Pass the collection ID to the edit modal
                        />
                      </Stack>
                    </TableCell>
                  </TableRow>
